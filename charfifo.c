...
//    while (btr > 0) {
//        char tmp = *(source->buf + read_idx);
//        *buf = tmp;
//        btr--;
//        read_idx++;
//        buf++;
//    }
    /* TODO: Something is wrong. memcpy failed on me before, for no reason, while a loop didn't, and then... memcpy didn't too. */
...
static void* RunThread(void* arg)
{
    ...
    for (;;) { /*TODO: Needs to exit when the context closes */
        ...
        size_t btr = bufUsedSpace(&thread_ctx->mem->to_server);
        if (btr > 0) {
...
static size_t ReadInput(threadCtx *thread_ctx)
{
    ...
    bufRead(&thread_ctx->mem->to_server, tmp, btr);
    tmp[btr] = '\0';
...
static void WriteOutput(threadCtx *thread_ctx)
{
    ...
    while(clientHasPendingReplies(c)) {
        if (c->bufpos > 0) {
            ...
            bufWrite(target, c->buf + c->sentlen, nwritten);
            if (nwritten <= 0) break;
            c->sentlen += nwritten;
            totwritten += nwritten;
            ...
            bufWrite(target, o + c->sentlen, nwritten);
//            nwritten = objlen - c->sentlen;
            if (nwritten <= 0) break;
            c->sentlen += nwritten;
            totwritten += nwritten;
